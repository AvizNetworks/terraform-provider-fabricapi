...
- **Fabric** (`fabricapi_fabric`): create/delete a fabric via the ONES UI config service (POST `/api/config/addFabricData`, DELETE `/api/config/deletefabricbyname/{name}`) — requires provider `config_endpoint` / `FABRIC_API_CONFIG_ENDPOINT`. Supports both east-west (`enable_ew` + `starting_subnet_gpu`) and north-south (`enable_ns` + required `starting_subnet_cpu`, with optional `starting_subnet_tenants` and `dedicated_storage`/`starting_subnet_storage`) networking
