...
- **North-South networking**: set `enable_ns = true` to enable the ONES UI's "N-S (Front-End) Network" section, matching `enable_ew`/`starting_subnet_gpu` for east-west. `frontend_storage = true` and `starting_subnet_cpu` are then **required**. `starting_subnet_tenants` (e.g. `"10.4"`, sent to the API as `"10.4.0"`) is optional. `dedicated_storage`/`starting_subnet_storage` is a separate optional pair (required together) that splits storage NICs onto their own subnet instead of sharing the CPU one. All off by default (east-west only, the prior behavior).
