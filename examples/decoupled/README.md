...
- **`is_ones_controlled`** (default `true`) matches the ONES UI's "Tenant control" radio (ONES vs External) — **unrelated** to `tenant_ctrl`, a separate, always-`"ones"` API field. `starting_subnet_tenants` only matters when `is_ones_controlled = false`: the UI hides that field entirely in ONES-managed mode, so the provider never sends it there either, and it becomes required (when `enable_ns` is also true) only once `is_ones_controlled` is set to `false`.
